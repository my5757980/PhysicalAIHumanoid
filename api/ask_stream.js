...
const COLLECTION_NAME = process.env.QDRANT_COLLECTION || "physical_ai_docs";
const COHERE_API_URL = "https://api.cohere.ai/v1";

// ---------- SAFETY CHECK ----------
function checkEnv() {
  if (!COHERE_API_KEY) throw new Error("COHERE_API_KEY is missing");
  if (!QDRANT_URL) throw new Error("QDRANT_URL is missing");
  if (!QDRANT_API_KEY) throw new Error("QDRANT_API_KEY is missing");
  if (!COLLECTION_NAME) throw new Error("QDRANT_COLLECTION is missing");
}
...
async function searchDocuments(query, maxResults = 5) {
  ...
  const res = await fetch(
    `${QDRANT_URL.replace(/\/$/, "")}/collections/${COLLECTION_NAME}/points/search`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
